...
import "./mainJs-61OQIpGh.js";
import "./globalUiJs-CvOT7_7X.js";
import { P as defineComponent, r as ref, v as onMounted, p as computed, c as createElementBlock, o as openBlock, d as createCommentVNode, a as createBaseVNode, t as toDisplayString, E as createVNode, a0 as withCtx, C as normalizeClass, F as withDirectives, G as vModelText, D as createApp } from "./chunks/runtime-dom.esm-bundler-cJ5wxxpw.js";
import { c as createPinia } from "./chunks/globalModal-BsIq6B_a.js";
import { E as EasyDataTableWrapper } from "./chunks/EasyDataTableWrapper-DdkOaPRe.js";
import { D as DeleteConfirmModal } from "./chunks/DeleteConfirmModal-CSkiauPP.js";
...
import "./chunks/TextInput-okLtxo6B.js";
const _hoisted_1 = {
  key: 0,
...
const _hoisted_4 = { class: "mb-4 flex justify-between items-center flex-wrap gap-2" };
const _hoisted_5 = { class: "flex gap-3 justify-center" };
...
const _hoisted_15 = { class: "flex flex-wrap gap-2 mt-4 mb-4" };
const _hoisted_16 = { class: "flex items-center gap-2 mb-4" };
  ...
  setup(__props) {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        initialItems.value = parcels.map((parcel) => apiService.transformParcelToOrder(parcel));
      } catch (err) {
    ...
    const openCreateModal = async () => {
      try {
        ...
        console.error("Create Modal cancelled:", error2);
      }
    };
    const openDeleteModal = async (item) => {
      try {
        ...
        if (result && result.type === "success" && result.data && result.data.success) {
          try {
            await apiService.deleteParcel(item.id);
            ...
            console.error("Ошибка удаления заказа:", error2);
            alert("Ошибка при удалении заказа. Попробуйте еще раз.");
          }
        }
      } catch (error2) {
        console.error("Delete Modal cancelled:", error2);
      }
    };
    const openDeleteSelectedModal = async () => {
      try {
        ...
        if (result && result.type === "success" && result.data && result.data.success) {
          try {
            const deletePromises = selectedItems.value.map((item) => apiService.deleteParcel(item.id));
            await Promise.all(deletePromises);
            const selectedIds = selectedItems.value.map((item) => item.id);
            initialItems.value = initialItems.value.filter((item) => !selectedIds.includes(item.id));
            selectedItems.value = [];
            alert("Выбранные заказы успешно удалены.");
          } catch (error2) {
            console.error("Ошибка удаления заказов:", error2);
            alert("Ошибка при удалении заказов. Попробуйте еще раз.");
          }
        }
      } catch (error2) {
        console.error("Delete Selected Modal cancelled:", error2);
      }
    };
    const viewOrderDetails = (item) => {
      window.location.href = `order-tracking.html?orderNumber=${item.number}`;
    };
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", null, [
        loading.value ? (openBlock(), createElementBlock("div", _hoisted_1, _cache[7] || (_cache[7] = [
          createBaseVNode("div", { class: "loading loading-spinner loading-lg text-primary" }, null, -1),
          createBaseVNode("p", { class: "mt-4 text-gray-600" }, "Загрузка заказов...", -1)
        ...
        !loading.value && !error.value ? (openBlock(), createElementBlock("div", _hoisted_3, [
          ...
          createVNode(EasyDataTableWrapper, {
            ...
            "item-number": withCtx((item) => [
              createBaseVNode("span", _hoisted_9, toDisplayString(item.number), 1)
            ]),
            "item-created": withCtx((item) => [
              createBaseVNode("span", _hoisted_10, toDisplayString(item.created), 1)
            ]),
            "item-from": withCtx((item) => [
              createBaseVNode("span", _hoisted_11, toDisplayString(item.from.region) + ", " + toDisplayString(item.from.city), 1)
            ]),
            "item-to": withCtx((item) => [
              createBaseVNode("span", _hoisted_12, toDisplayString(item.to.region) + ", " + toDisplayString(item.to.city), 1)
            ]),
            "item-pickupDate": withCtx((item) => [
              createBaseVNode("span", _hoisted_13, toDisplayString(item.pickupDate), 1)
            ]),
            "item-deliveryDate": withCtx((item) => [
              createBaseVNode("span", _hoisted_14, toDisplayString(item.deliveryDate), 1)
            ]),
...
const OrdersPage = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-92a78bb9"]]);
