...
class ApiService {
  ...
  async getTariffGrids(transportTypeId = null) {
    try {
      const url = transportTypeId ? `${this.baseUrl}/tariffGrids?transportType_id=${transportTypeId}` : `${this.baseUrl}/tariffGrids`;
  ...
  async getTariffZones(transportTypeId = null) {
    try {
      const url = transportTypeId ? `${this.baseUrl}/tariffZones?transportType_id=${transportTypeId}` : `${this.baseUrl}/tariffZones`;
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  async getBillingAddressesWithRelations() {
    try {
      ...
      const enrichedAddresses = billingAddresses.map((address) => {
        ...
        const region = (locality == null ? void 0 : locality.region) || null;
        return {
          ...address,
