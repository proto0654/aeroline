/**
 * autocomplete.js
 * Модуль для создания полей ввода с автокомплитом
 */

/**
 * Создание поля ввода с автокомплитом
 * @param {HTMLElement} container - контейнер для размещения поля
 * @param {Array} offices - массив офисов для автокомплита
 * @param {string} id - идентификатор для элементов
 * @param {string} placeholder - текст placeholder
 * @param {Object} options - дополнительные опции
 * @param {boolean} options.onlyCities - показывать только города без адресов
 */
export function createAutocompleteInput(container, offices, id, placeholder, options = {}) {
  console.log(`Создание поля автокомплита ${id} с плейсхолдером "${placeholder}"`);
  
  // Сохраняем иконку (стрелку) из существующего select
  const existingArrow = container.querySelector('img');
  let arrowHTML = '';
  if (existingArrow) {
    const arrowSrc = existingArrow.src;
    arrowHTML = `<img src="${arrowSrc}" alt="" class="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-none z-10">`;
  }
  
  // Создаем разметку для поля с автокомплитом
  const autocompleteHTML = `
    <div class="relative w-full">
      <input 
        id="${id}-input" 
        type="text" 
        class="appearance-none bg-white border border-gray-200 rounded-lg px-4 py-2 w-full focus:outline-none text-body-secondary !pr-8 text-sm"
        placeholder="${placeholder}" 
        autocomplete="off"
        required
      >
      <button type="button" class="absolute right-0 top-0 h-full w-12 text-gray-400 focus:outline-none z-10"></button>
      ${arrowHTML}
      <ul 
        id="${id}-suggestions" 
        class="absolute w-full bg-white border border-gray-200 rounded-md shadow-md mt-1 max-h-60 overflow-y-auto hidden z-20"
        style=""
      ></ul>
    </div>
  `;
  
  // Заменяем содержимое контейнера
  container.innerHTML = autocompleteHTML;
  console.log(`Вставлена HTML-разметка для поля ${id}`);
  
  // Получаем ссылки на новые элементы
  const input = document.getElementById(`${id}-input`);
  const list = document.getElementById(`${id}-suggestions`);
  const toggleButton = container.querySelector('button');
  
  if (!input || !list || !toggleButton) {
    console.error(`Не удалось создать элементы автокомплита для ${id}`, {
      input: !!input,
      list: !!list,
      toggleButton: !!toggleButton
    });
    return;
  }
  
  console.log(`Созданы элементы автокомплита для ${id}`);
  
  let currentIndex = -1;
  
  // Формирует строку для подстановки в input
  function officeToString(office) {
    if (options.onlyCities) {
      return office.city;
    }
    return `${office.city}, ${office.address}${office.phone ? ', ' + office.phone : ''}`;
  }

  // Формирует HTML для подсказки
  function officePreviewHTML(office) {
    if (options.onlyCities) {
      return `
        <div style="line-height:1.3; padding:2px 0;">
          <div style="font-weight:bold;">${office.city}</div>
        </div>
      `;
    }
    return `
      <div style="line-height:1.3; padding:2px 0;">
        <div style="font-weight:bold;">${office.city}</div>
        <div>${office.address}</div>
        <div style="color:#888; font-size:0.95em;">${office.type || ''}</div>
        <div style="color:#008DD2; font-size:0.98em;">${office.phone || ''}</div>
      </div>
    `;
  }

  // Фильтрация офисов по всем полям с приоритетом: сначала city.startsWith, потом city.includes, потом остальные
  function filterOffices(query) {
    const q = query.toLowerCase().trim();
    
    if (options.onlyCities) {
      // В режиме только городов фильтруем только по названию города
      const byCityStart = new Map(); // Используем Map для уникальности
      const byCityInclude = new Map(); // Используем Map для уникальности
      
      // Сначала собираем все уникальные города
      const uniqueCities = new Map();
      offices.forEach(office => {
        if (office.city) {
          const cityName = office.city.trim();
          const cityLower = cityName.toLowerCase();
          if (!uniqueCities.has(cityLower)) {
            uniqueCities.set(cityLower, office);
          }
        }
      });
      
      // Теперь фильтруем уникальные города
      uniqueCities.forEach((office, cityLower) => {
        if (cityLower.startsWith(q)) {
          byCityStart.set(cityLower, office);
        } else if (cityLower.includes(q)) {
          byCityInclude.set(cityLower, office);
        }
      });
      
      // Преобразуем Map обратно в массив
      return [...byCityStart.values(), ...byCityInclude.values()];
    }
    
    const byCityStart = [];
    const byCityInclude = [];
    const byOther = [];
    offices.forEach(office => {
      if (office.city && office.city.toLowerCase().startsWith(q)) {
        byCityStart.push(office);
      } else if (office.city && office.city.toLowerCase().includes(q)) {
        byCityInclude.push(office);
      } else if (
        (office.address && office.address.toLowerCase().includes(q)) ||
        (office.type && office.type.toLowerCase().includes(q)) ||
        (office.phone && office.phone.toLowerCase().includes(q))
      ) {
        byOther.push(office);
      }
    });
    return [...byCityStart, ...byCityInclude, ...byOther];
  }

  // Обновление списка подсказок
  function updateSuggestions(filtered) {
    list.innerHTML = '';
    currentIndex = -1;
    filtered.forEach((office, index) => {
      const li = document.createElement('li');
      li.innerHTML = officePreviewHTML(office);
      li.className = 'px-4 py-2 hover:bg-blue-100 cursor-pointer';
      li.addEventListener('click', () => {
        // Сначала устанавливаем значение в поле ввода
        input.value = officeToString(office);
        list.classList.add('hidden');
        
        // Затем диспатчим событие выбора города асинхронно через Promise
        if (options.onlyCities) {
          Promise.resolve()
            .then(() => {
              const event = new CustomEvent('citySelected', { detail: { city: office.city } });
              input.dispatchEvent(event);
            });
        }
      });
      list.appendChild(li);
    });
    list.classList.remove('hidden');
  }

  // Показать все офисы
  function showAllSuggestions() {
    list.innerHTML = '';
    currentIndex = -1;
    
    if (options.onlyCities) {
      // В режиме только городов показываем только уникальные города
      const uniqueCities = new Map();
      offices.forEach(office => {
        if (office.city) {
          const cityName = office.city.trim();
          const cityLower = cityName.toLowerCase();
          if (!uniqueCities.has(cityLower)) {
            uniqueCities.set(cityLower, office);
          }
        }
      });
      
      uniqueCities.forEach((office, index) => {
        const li = document.createElement('li');
        li.innerHTML = officePreviewHTML(office);
        li.className = 'px-4 py-2 hover:bg-blue-100 cursor-pointer';
        li.addEventListener('click', () => {
          input.value = officeToString(office);
          list.classList.add('hidden');
          
          // Диспатчим событие выбора города асинхронно через Promise
          if (options.onlyCities) {
            Promise.resolve()
              .then(() => {
                const event = new CustomEvent('citySelected', { detail: { city: office.city } });
                input.dispatchEvent(event);
              });
          }
        });
        list.appendChild(li);
      });
    } else {
      // В обычном режиме показываем все офисы
      offices.forEach((office, index) => {
        const li = document.createElement('li');
        li.innerHTML = officePreviewHTML(office);
        li.className = 'px-4 py-2 hover:bg-blue-100 cursor-pointer';
        li.addEventListener('click', () => {
          input.value = officeToString(office);
          list.classList.add('hidden');
          
          // Диспатчим событие выбора города асинхронно через Promise
          if (options.onlyCities) {
            Promise.resolve()
              .then(() => {
                const event = new CustomEvent('citySelected', { detail: { city: office.city } });
                input.dispatchEvent(event);
              });
          }
        });
        list.appendChild(li);
      });
    }
    
    list.classList.remove('hidden');
  }

  // Кнопка-стрелка
  toggleButton.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (list.classList.contains('hidden')) {
      showAllSuggestions();
    } else {
      list.classList.add('hidden');
    }
  });

  // Ввод текста
  input.addEventListener('input', () => {
    const value = input.value.trim();
    if (!value) {
      list.classList.add('hidden');
      return;
    }
    const filtered = filterOffices(value);
    if (filtered.length === 0) {
      list.classList.add('hidden');
      return;
    }
    updateSuggestions(filtered);
  });

  // Клавиши
  input.addEventListener('keydown', (e) => {
    const items = list.querySelectorAll('li');
    if (list.classList.contains('hidden') || items.length === 0) {
      if (e.key === 'ArrowDown' && input.value.trim() === '') {
        e.preventDefault();
        showAllSuggestions();
        return;
      }
      return;
    }
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      currentIndex = (currentIndex + 1) % items.length;
      highlightItem(items);
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      currentIndex = (currentIndex - 1 + items.length) % items.length;
      highlightItem(items);
    } else if (e.key === 'Enter') {
      e.preventDefault();
      if (currentIndex >= 0 && currentIndex < items.length) {
        const office = filterOffices(input.value.trim())[currentIndex] || offices[currentIndex];
        if (office) input.value = officeToString(office);
        list.classList.add('hidden');
      }
    } else if (e.key === 'Escape') {
      e.preventDefault();
      list.classList.add('hidden');
    }
  });

  // Скрыть при клике вне
  document.addEventListener('click', (e) => {
    if (!list.contains(e.target) && e.target !== input && e.target !== toggleButton) {
      list.classList.add('hidden');
    }
  });

  // Показывать при фокусе, если есть текст
  input.addEventListener('focus', () => {
    const value = input.value.trim();
    if (!value) return;
    const filtered = filterOffices(value);
    if (filtered.length > 0) {
      updateSuggestions(filtered);
    }
  });

  // Стилизация списка
  list.style.maxHeight = '200px';
  list.style.overflowY = 'auto';
  list.style.overscrollBehavior = 'contain';
}

/**
 * Функция подсветки выбранного элемента
 * @param {HTMLElement} items - список элементов для подсветки
 */
function highlightItem(items) {
  items.forEach((item, index) => {
    item.classList.toggle('bg-blue-100', index === currentIndex);
  });
  
  // Прокрутка до выделенного элемента, если он не виден
  if (currentIndex >= 0) {
    const selectedItem = items[currentIndex];
    const listRect = list.getBoundingClientRect();
    const itemRect = selectedItem.getBoundingClientRect();
    
    if (itemRect.bottom > listRect.bottom) {
      // Если элемент ниже видимой области - прокручиваем вниз
      selectedItem.scrollIntoView(false);
    } else if (itemRect.top < listRect.top) {
      // Если элемент выше видимой области - прокручиваем вверх
      selectedItem.scrollIntoView(true);
    }
  }
} 