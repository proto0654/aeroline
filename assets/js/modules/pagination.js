...
export class Pagination {
  ...
  constructor(options) {
    ...
    if (this.container && this.paginationContainer) {
      ...
      console.error(
        `Pagination: Контейнер "${this.containerSelector}" найден: ${
          this.container !== null
        }`
      );
      console.error(
        `Pagination: Контейнер пагинации "${this.paginationSelector}" найден: ${
          this.paginationContainer !== null
        }`
      );
    }
  ...
  createPaginationElements() {
    try {
      ...
      prevButton.className =
        "pagination-prev pagination-button flex items-center justify-center rounded bg-brand-light text-brand-gray hover:bg-brand-blue hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors w-8 h-8";
      ...
      nextButton.className =
        "pagination-next pagination-button flex items-center justify-center rounded bg-brand-light text-brand-gray hover:bg-brand-blue hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors w-8 h-8";
  ...
  addPageButton(page, currentPage, nav, nextButton) {
    ...
    if (page === currentPage) {
      pageButton.className =
        "pagination-number pagination-button flex items-center justify-center rounded bg-brand-blue text-white font-bold w-8 h-8";
    } else {
      pageButton.className =
        "pagination-number pagination-button flex items-center justify-center rounded bg-brand-light text-brand-gray hover:bg-brand-blue hover:text-white transition-colors w-8 h-8";
    }
  ...
  addEllipsis(nav, nextButton) {
    ...
    ellipsis.className =
      "pagination-ellipsis flex items-center justify-center w-8 h-8 text-brand-gray";
  ...
  applyFilter(filterValue) {
    ...
    // Теперь нам не нужно искать элемент, мы получаем значение напрямую
    // const filterInput = document.querySelector(this.filterSelector);
    // if (!filterInput) {
