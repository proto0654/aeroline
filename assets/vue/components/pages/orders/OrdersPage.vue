<template>
  <div>
    ...
    <div v-if="error" class="alert alert-error mb-4">
      ...
      <span>{{ error }}</span>
      <button class="btn btn-sm btn-outline" @click="loadOrders">Повторить</button>
    </div>
...
const loadOrders = async () => {
  ...
  try {
    ...
    // Преобразуем данные посылок в формат заказов
...
const viewOrderDetails = (item: any) => {
  // Перенаправляем на страницу отслеживания с номером заказа
